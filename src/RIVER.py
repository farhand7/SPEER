...
def main():
    ...
    genomeonly_sharedtissue_beta = trainSharedGenomeOnlyModel(annotations, expression, tissue_groups)
    c = 0
    #tissues = list(expression.columns)[0:43]
    for group in tissue_groups:
    #for tissue in tissues:
        ...
        #expr = np.abs(expression[tissue_groups[group]])
        #tissue = tissue_groups[group][0]
        #expr = np.abs(expression[tissue])
        
        #else:
            #expr_group = expression[tissue_groups[group]].dropna(thresh = 2)
            #expr = np.abs(expr_group).median(axis=1)
