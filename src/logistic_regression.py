...
def _cost_function(vBeta, vBetaParent, mX, vY, lambda_hp):
	# cost function
	xr = (-(np.sum(vY*(np.dot(mX, vBeta) - np.log((1.0 + np.exp(np.dot(mX, vBeta))))) + 
		(1-vY)*(-np.log((1.0 + np.exp(np.dot(mX, vBeta)))))))) + .5 * lambda_hp * (vBeta[1:] - vBetaParent[1:]).dot((vBeta[1:] - vBetaParent[1:]))
...
def sgd(G, Q, beta_c, beta_parent, lambda_hp):
	...
	vY = Q
	lambda_hp = lambda_hp
	beta_c = scipy.optimize.fmin_bfgs(_cost_function, x0 = vBeta, fprime = _gradient_function, args = 
		(vBetaParent, mX, vY, lambda_hp), gtol = 1e-3, disp=False)	
	return beta_c
